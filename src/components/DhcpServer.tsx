...
export default function DhcpServer({
    ...
    try {
      ...
      for (let i = 0; i < limit; i++) {
        ...
        if (lease) {
          ...
          status = 'reserved';
          hostname = `${reservation.hostname} (고정 예약)`;
        }
  ...
  const handleSaveConfig = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.success === false) {
        ...
        setFeedback({ type: 'success', message: 'DHCP 설정이 적용되었습니다.' });
      }
    } catch (err) {
      setFeedback({ type: 'error', message: '설정 적용 중 오류가 발생했습니다.' });
    } finally {
    ...
    if (!dhcpRunning) {
      ...
      if (!alreadyAcknowledged) {
        const confirmed = window.confirm(
          '이 네트워크에 이미 DHCP 서버(공유기 등)가 있으면 충돌할 수 있습니다. 계속할까요?'
        );
        if (!confirmed) return;
  ...
  const runNeighborQuery = async (payload: Record<string, unknown>) => {
    ...
    try {
      ...
      setNeighborResult({ success: false, error: err?.message || '요청 중 오류가 발생했습니다.' });
    } finally {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              <strong className="text-indigo-400 font-mono">{config.interfaceName}</strong>로 로컬 단말에 IP를 자동 할당합니다.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              가동 중이면 4초마다 단말을 자동 탐지하고, 6초마다 온라인 상태를 갱신합니다.
            </p>
          </div>
          <button
            ...
            title={dhcpRunning ? "지금 다시 스캔" : "서비스를 먼저 가동하세요"}
          >
      ...
      <form onSubmit={handleSaveConfig} className="p-4 glass-card rounded-2xl space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-850 pb-1.5 gap-1.5">
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">IP 대역과 서브넷 마스크를 설정합니다.</p>
          </div>
      ...
      <div className="space-y-4">
        ...
        <div className="p-4 glass-card rounded-xl space-y-3">
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              IP 임대 풀(<strong className="text-emerald-400 font-mono">{rangeStart} ~ {rangeEnd}</strong>)의 사용 현황입니다.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-1.5 border-b border-slate-850/60">
            <div>
              <h3 className="text-xs font-display font-bold text-white flex items-center gap-1.5">
                ...
                할당 단말 현황 ({filteredLeases.length}개)
              </h3>
                  ...
                  {filteredLeases.map((lease) => {
                    ...
                    return (
                      <tr key={lease.id} className="hover:bg-slate-900/10 transition">
                        ...
                        <td className="p-2">
                          <span className="flex items-center gap-1.5" title={lease.lastCheckedAt ? `마지막 확인: ${new Date(lease.lastCheckedAt).toLocaleTimeString()}` : '아직 미확인'}>
        ...
        <div className="p-4 glass-card rounded-xl space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              ...
              <p className="text-[10px] text-slate-400 mt-0.5">
                정적 IP 단말을 포함해 네트워크의 모든 단말을 보여줍니다. 5초마다 자동 갱신됩니다.
              </p>
            </div>
            {arpLoading && (
                ...
                {arpEntries.length === 0 && (
                  <tr>
                    <td colSpan={3} className="text-center text-slate-500 py-6 text-xs">
                      {arpLoading ? 'ARP 테이블 조회 중...' : '탐지된 단말이 없습니다.'}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="p-4 glass-card rounded-xl space-y-4">
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">장비 MAC에 고정 IP를 매핑합니다 (예: DB, 프린터, 스위치).</p>
          </div>
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-850/30 text-slate-300 text-[11px]">
                  ...
                  {reservations.length === 0 && (
                    <tr>
                      <td colSpan={4} className="text-center text-slate-500 py-6">등록된 고정 IP가 없습니다.</td>
                    </tr>
                  )}
                </tbody>
              </table>
              ...
              <div>
                ...
                {neighborModal.mode === 'result' && (
                  <p className="text-[10px] text-slate-500 mt-0.5">
                    일반 PC/서버는 "지원하지 않는 명령" 출력이 나올 수 있습니다 (정상).
                  </p>
                )}
                {neighborModal.mode === 'form' && (
                  <p className="text-[10px] text-slate-500 mt-0.5">
                    등록되지 않은 장비입니다. 접속 정보를 입력하세요 (저장되지 않고 1회만 사용).
                  </p>
                )}
              </div>
              <button
                ...
                <form onSubmit={handleSubmitAdhocNeighborQuery} className="space-y-3 text-xs" id="adhoc-neighbor-cred-form">
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <div className="col-span-2">
                      <label className="block text-slate-300 font-bold mb-1">포트</label>
                      <input
                        type="number"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-slate-300 font-bold mb-1">사용자</label>
                      <input
                        type="text"
                        className="w-full bg-slate-900 border border-slate-800 rounded-lg p-2 text-white focus:outline-none"
                        placeholder="admin"
                    ...
                    <div>
                      <label className="block text-slate-300 font-bold mb-1">비밀번호</label>
                      <input
                        type="password"
                        className="w-full bg-slate-900 border border-slate-800 rounded-lg p-2 text-white focus:outline-none"
                        placeholder="••••••••"
