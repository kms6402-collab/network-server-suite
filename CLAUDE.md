...
- `dhcpConfig.interfaceName`의 기본값은 `getDefaultInterfaceName()`이 `os.networkInterfaces()`로 찾은 첫 번째 실제(non-internal) IPv4 어댑터로 자동 설정된다. 서버 기동 시 `loadState()` 이후, 저장된 어댑터가 현재 호스트에 없으면(다른 PC의 설정을 그대로 들고 온 경우 등) 자동으로 실제 어댑터 기준으로 self-heal한다(`ensureDhcpConfigMatchesHost()`, DHCP 토글 on 시점에도 재검증).
- `getInterfaceInfo(name)`은 어댑터를 찾지 못하면 가짜 IP/MAC을 반환하지 않고 `null`을 반환한다 — 호출부는 반드시 null 체크를 해야 한다.
- **DHCP 서버 자신의 IP와 클라이언트에게 알려주는 게이트웨이는 서로 다른, 독립적인 값이다(중요, 되돌리지 말 것)**: 과거에는 `dhcpConfig.gateway`가 곧 "이 앱 자신의 IP"를 의미했다 — `getSubnetDefaults()`가 게이트웨이 기본값을 어댑터의 실제 IP로 채웠고, `ensureGatewayAliasOnAdapter()`가 관리자가 입력한 게이트웨이 값을 어댑터에 보조(alias) IP로 강제로 붙여서 "게이트웨이 = 이 PC 자신"이 항상 참이 되도록 만들었다. 그 결과 DHCP로 IP를 받은 단말은 게이트웨이가 실제 라우터가 아니라 이 DHCP 서버 PC 자신이 되어버렸고, 이 앱은 IP 포워딩/NAT을 하지 않으므로 그 단말은 게이트웨이를 통한 정상적인 통신(인터넷/다른 대역 접근)이 불가능했다. 지금은 완전히 분리되어 있다:
  - **DHCP 서버 IP**(옵션 54/siaddr — 클라이언트에게 "이 서버가 나다"라고 알려주는 값)는 항상 `getInterfaceInfo(dhcpConfig.interfaceName)`이 반환하는, 선택된 어댑터의 실제 현재 IP를 그대로 쓴다. 저장되는 설정값이 아니라 매 요청마다 `getCurrentDhcpServerIp()`로 즉석에서 계산되며, `/api/status`·`/api/dhcp/toggle`·`/api/dhcp/config` 응답의 `dhcpServerIp` 필드로 노출되고 `DhcpServer.tsx`에 읽기 전용 "서버 IP" 배지로 표시된다.
  - **`dhcpConfig.gateway`**(옵션 3/router — 클라이언트가 실제로 트래픽을 보낼 라우터 주소)는 완전히 독립적인, 관리자가 직접 입력하는 값이다. `getSubnetDefaults()`의 기본값은 이제 어댑터 IP가 아니라 그 서브넷의 관용적인 `.1`이다(다른 DHCP 서버들과 동일한 방식의 추측값일 뿐, 바로 저장/사용 가능하지만 실제 라우터 주소로 바꿔야 한다). `DhcpServer.tsx`의 어댑터 변경 핸들러(`handleInterfaceChange`)도 동일하게 `.1`을 기본값으로 채운다.
  - 이 분리 덕분에 게이트웨이를 어댑터에 보조 IP로 강제로 붙일 필요가 완전히 없어졌다 — `ensureGatewayAliasOnAdapter`/`removeGatewayAlias`/`managedGatewayAlias`/`subnetMaskToPrefixLength`/`escapePowerShellSingleQuoted`/`getAllInterfaceIPv4s`(PowerShell `New-NetIPAddress`·`Remove-NetIPAddress`·netsh 폴백을 쓰던 코드 전체)는 죽은 코드가 되어 완전히 제거되었다. `getInterfaceInfo(name)`도 더 이상 `preferredIp` 매개변수를 받지 않는다(예전엔 어댑터가 들고 있는 여러 IPv4 주소 중 "게이트웨이 alias와 일치하는 것"을 찾기 위한 것이었는데, 그 alias 자체가 없어졌으므로 무의미해졌다) — 그냥 어댑터의 첫 번째 non-internal IPv4를 반환한다.
  - `/api/dhcp/config`는 더 이상 게이트웨이가 바뀌었다고 어댑터에 뭔가를 추가하지 않는다 — DHCP가 이미 가동 중일 때 `interfaceName`이 바뀐 경우에 한해 `host-pc-self` 리스 항목을 새 어댑터의 실제 IP로 갱신할 뿐이다.
