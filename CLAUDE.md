...
- **`webAuth`(계정 정보)는 `applet_state.json`과 `setting.ini` 두 파일에 이중으로 저장된다 — 계정을 완전히 지우려면 반드시 둘 다 없어져야 한다(중요, 비직관적)**: `saveState()`는 매번 `applet_state.json`(주 저장소, JSON)과 `setting.ini`(사람이 읽을 수 있는 보조 백업, `[WebAuth] data=`)를 항상 함께 쓴다. `loadState()`는 `applet_state.json`이 있으면 그것만 쓰고, 그게 없을 때만 `setting.ini`로 폴백한다 — 즉 사용자가 둘 중 **하나만** 수동으로 지우면 나머지 파일에 남아있는 `webAuth`(비밀번호 해시+salt 포함)가 그대로 복원되어, "설정 파일을 지웠는데 로그인 계정이 그대로 남아있다"는 버그처럼 보이는 상황이 생긴다(실제 사용자 리포트로 확인됨). 이건 의도된 이중 백업 설계이므로 `loadState()`의 우선순위 자체를 바꾸지 말 것 — 대신 계정을 실제로 초기화하려면 앱 안의 "설정 → 서버 초기화(공장 초기화)" 버튼(`POST /api/system/reset`)을 쓰거나, 수동으로 지울 거면 두 파일을 **동시에** 지워야 한다. `POST /api/system/reset`은 `STATE_FILE`뿐 아니라 `SETTINGS_FILE`도 명시적으로 `fs.unlinkSync`한다(과거엔 `STATE_FILE`만 지우고 `setting.ini`는 뒤이은 `saveState()`가 `webAuth: null`로 덮어쓰는 것에만 의존했다 — 그 덮어쓰기 자체가 실패하면 지워지지 않은 예전 계정 해시가 그대로 디스크에 남을 수 있었다).
...
- **진짜 DHCP 서버가 구현되어 있다** (`server.ts`의 "ADAPTER ISOLATION" 주석 블록부터 시작하는 섹션, `startDhcpServer`/`stopDhcpServer`, DISCOVER/REQUEST/RELEASE 핸들러). `/api/dhcp/toggle`이 `enabled:true`가 되면 실제 UDP 소켓을 열어 DORA(Discover-Offer-Request-Ack) 프로토콜을 처리하고, `enabled:false`면 소켓을 닫는다. Node `dgram`으로 직접 구현했다(마땅한 유지보수 중인 npm 패키지가 없었음, RFC 2131/2132 패킷을 직접 파싱/생성). 새 의존성 없음.
  ...
  - **`discoverNetworkDevices()`의 "이미 추적 중인가" 판단에서 `status !== 'expired'` 조건을 빼야 한다(중요, 되돌리지 말 것)**: 예전에는 이 조건이 있어서, 실제 DHCP 클라이언트가 없어 갱신되지 않는 ARP 전용 추적 기기(고정 IP를 쓰는 장비 등)가 `dhcpConfig.leaseTime`마다 `expireLeases()`에 의해 'expired'로 표시될 때마다, 바로 다음 ARP 스캔 틱(가동 중 4초 간격)이 그 기기를 "처음 보는 기기"로 오판해서 매번 새 "Device-X" 항목을 또 만들었다 — 같은 IP/MAC을 가진 항목이 임대 시간 간격으로 계속 쌓이는 버그의 원인이었다(할당 단말 현황과 CSV 내보내기에 중복으로 표시됨). 지금은 상태와 무관하게 MAC이 이미 있으면 그 자리에서 되살리기만 한다(상태를 'active'로, `leasedAt`/`expiresAt`를 갱신) — 새 항목을 만들지 않는다.
  - **`dedupeLeases()`**: 같은 (정규화된 MAC, IP) 조합을 가진 리스가 여러 개 있으면 하나만 남기고 정리한다(우선순위: `reserved` > `active` > `expired`, 그다음 `Device-N` 같은 일반 이름이 아닌 실제 호스트명, 그다음 더 최근에 임대된 것). 위 버그가 고쳐지기 전에 이미 쌓인 중복 데이터를 정리하는 역할도 겸해서 `loadState()`가 `leases`를 복원한 직후 한 번 호출되고, DHCP 가동 중에는 2초 틱커에서 `expireLeases()`와 함께 계속 호출된다(혹시 모를 다른 경로의 중복 생성에 대한 방어선). 같은 IP에 서로 다른 MAC이 있는 경우(실제 IP 충돌/이상 상황)는 절대 하나로 합치지 않는다 — 그건 감춰야 할 중복이 아니라 사용자에게 보여줘야 할 진짜 이상 신호다.
  ...
  - "할당 단말 현황"의 각 동적 리스 행에 핀(📌) 아이콘 버튼이 있다(`handlePinLeaseAsReservation`) — 클릭하면 그 리스의 현재 mac/ip/hostname 그대로 `onAddReservation`(기존 예약 추가 API)을 호출해 즉시 고정 예약으로 승격한다. 이미 `status === 'reserved'`인 행(`host-pc-self` 포함)에는 표시되지 않는다.
  - **CDP/LLDP 이웃 조회(`queryNeighborInfo`)**: SSH/Telnet으로 일회성 접속해 `show cdp neighbors detail` → `show lldp neighbors detail` → `show lldp neighbors`(마지막 것은 "detail" 키워드를 못 알아듣는 Extreme Networks/EXOS 장비용, Cisco류 장비에서는 그냥 무시되거나 에러로 echo됨 — 기존 두 명령과 동일한 방식으로 안전) 세 명령을 순서대로 흘려보내고 원문 출력을 그대로 사용자에게 보여준다. 장비가 지원하지 않는 명령은 "command not found"류 텍스트가 섞여 나오는 게 정상이며 요청 실패로 취급하지 않는다.
...
- **원격 stream 출력은 줄 단위로 버퍼링해서 `logs`에 쌓는다(중요, 되돌리지 말 것)**: `ScriptExecution.logs: string[]`의 각 항목은 프론트에서 독립된 한 줄(block-level `<div>`)로 렌더링되는데, 예전에는 SSH `stream.on("data", ...)`/Telnet `connection.on("data", ...)`이 받은 raw TCP chunk를 `appendLog()`로 그대로 `logs.push()`했다 — 일부 장비(Extreme/WiNG 계열 AP 등)는 키 입력 echo나 응답을 실제 줄바꿈과 무관한 임의 크기의 조각으로 나눠 보내서, 화면에서 한 줄이 여러 줄로 쪼개져 보이는("깨져 보인다") 버그가 있었다. 지금은 `appendStreamChunk(execId, chunk)`가 이 두 곳(및 stderr)에서만 쓰이며, `openStreamLineExecIds: Set<execId>`로 "마지막 `logs` 항목이 아직 줄바꿈으로 안 닫힌 상태(open)"인지 추적해서, open 상태면 새 chunk를 새 항목으로 push하지 않고 마지막 항목에 이어 붙이고, 실제 `\n`을 만나야 그 지점에서 새 항목을 시작한다. 반대로 상태 메시지/명령 echo/수동 CLI echo처럼 원래도 항상 완결된 한 줄이던 `appendLog()`/`logs.push()` 호출들은 여전히 매번 새 항목을 만들되, 그 전에 `openStreamLineExecIds`를 지워서 직전에 열려 있던 raw stream 줄에 잘못 이어 붙는 일이 없게 한다. `closeLiveSession()`도 세션 종료 시 이 Set 엔트리를 정리한다.
...
- **세션 탭 자동 전환은 사용자가 직접 탭을 클릭하면 멈춘다(중요, 되돌리지 말 것)**: `TerminalAutomation.tsx`의 자동 추적 `useEffect`는 원래 `executions`가 갱신될 때마다(1.5초 폴링 틱마다, 또는 순차 배치 실행이 다음 호스트로 넘어갈 때마다) `status === 'running'`인 세션으로 활성 탭을 강제로 되돌렸다 — 사용자가 이미 끝난 세션 탭을 보고 있어도 매번 화면이 초기화되는 버그였다. `userPinnedTabRef`(리렌더에 영향받지 않는 ref)로 "사용자가 탭을 직접 클릭했는지"를 추적해서, pinned 상태면 이 effect가 활성 탭을 건드리지 않는다(`handleSelectTab`이 탭 클릭 시 pin을 세움). 반대로 "명시적으로 새 실행을 시작"하는 신호(`handleRunScript`, `handleRunSavedBatchJob`, 수동 접속 버튼의 `onClick`)는 pin을 해제해서 새로 시작하는 세션을 다시 따라가게 한다. 활성 탭이 목록에서 사라진 경우(사용자가 탭을 닫음)는 pin 여부와 무관하게 다른 탭 또는 null로 복구한다.
